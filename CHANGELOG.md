...
- **API break: `rimap_audit`'s three non-record `pub` structs are now
  ...
  `AuditOptions` gains `AuditOptions::new(path, initial_seq)` — the two fields
  with no defensible default — and leaves the rest inert: rotation disabled,
  no retention, fail-closed. `initial_seq` is a parameter rather than a
  defaulted `Seq::FIRST` for the reason `ProcessEnd::new` takes
  `records_lost`: `AuditWriter::open` plumbs it into the sequence counter and
  checks it against nothing, so a defaulted first-record value is an
  affirmative claim that this is a fresh chain, and a caller who never checked
  would put duplicate `seq` values into an append-only log. Continuing an
  existing file means passing
  `read_trailing_state(path)?.last_seq.map_or(Seq::FIRST, Seq::next)`.

  The rotation and retention fields are deliberately *not* the config-layer
  defaults (10 MiB / keep 5), which belong to
  `rimap_config::model::AuditConfig::new`; a caller wanting them reads a
  loaded config, as the server's boot path does. Note the two same-named
  constructors differ on purpose. Depart from the inert values by assignment:
  `let mut o = AuditOptions::new(path, Seq::FIRST); o.rotate_bytes = …;`.

  `Filter` and `TrailingState` gain no constructor — both already derive
  `Default`, which is the right starting point for each (all-`None` "match
  everything", and "nothing readable" respectively), and `TrailingState` is an
  output type callers read from `read_trailing_state` rather than build.
