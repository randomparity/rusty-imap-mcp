...
fn dump_tool_catalog_emits_object_schemas() {
    ...
    // 27 ToolName variants minus 3 sub-capabilities (SearchAdvanced,
    // FetchMessageHtml, CreateDraftHtml) that share schemas with their
    // parents = 24 defs.
    assert_eq!(
        lines.len(),
        24,
        "expected 24 tool defs (27 ToolName variants - 3 sub-capabilities); got {}",
        lines.len(),
    );
