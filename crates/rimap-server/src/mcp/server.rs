...
use crate::mcp::drain::DispatchDrain;
...
impl ServerHandler for ImapMcpServer {
    ...
    async fn list_tools(
        ...
        // Honor `tools/list` cursor pagination (#411): a large multi-account
        // catalog is served one page at a time instead of as a single
        // response. The cursor is an opaque decimal offset into the
        // deterministically-ordered catalog; a well-behaved client walks
        // `next_cursor` to completion. A single- or few-account catalog fits
        // in one page (`next_cursor` absent), so those deployments see no
        // behavior change.
        let start = match request.and_then(|r| r.cursor) {
            Some(cursor) => decode_tool_cursor(&cursor)?,
            None => 0,
        };
        let (start, end, next) = tool_page_window(tools.len(), start);
