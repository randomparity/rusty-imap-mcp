//! Top-level `parse_message` pipeline + the six MIME-parse hard
//! limits. Routes raw RFC 5322 bytes through `mime_scrub` →
//! `safe_parser` → header / body / meta extraction → `lookalike::audit`,
//! returning a [`Content`] structure.
