...
- **Committed per-test timing table.** verified: JUnit XML carries per-test
  ...
  durations every run (nextest docs, nexte.st/docs/reporting +
  /machine-readable/junit), so a static table rots for no informational gain.
- **`test-timing` convenience recipe.** verified: jq cannot read the XML
  report (`printf '<testsuites>…' | jq '.testsuites'` → parse error, exit 5),
  and shell-side XML attribute extraction via xmllint is disproportionate for
  advisory data — the run-tail slow listing already carries durations, and
  junit.xml is directly available to any agent tool that speaks XML.
  judgment: cut while cheap.
